...
//  $Id: pdep.cc 1876 2015-01-29 11:47:54Z ynaga $
...
namespace pdep {
  ...
  inline void parser::_add_global_feature (const chunk_t* const bi, const chunk_t* const bj) {
    ...
    ny::uint bracket_in (0), bracket_out (0), period (0), comma (0);
  ...
  void parser::_event_gen_from_tuple (const int i) { // for chunking
    ...
    if (fmax && _fi != fmax)
      err (1, HERE "feature offset broken; revert the change in features.\n");
    fmax = _fi;
  }
...
#endif
  const sentence_t* parser::parse_from_postagged (char* result, size_t len) {
    _s->clear (true);
    if (! len) len = std::strlen (result);
    //
    char *r = result + len - 4;
    if (len < 4 || *r != 'E' || *(r+1) != 'O' || *(r+2) != 'S' || *(r+3) != '\n')
      err (1, HERE "found a tagged sentence that is not EOS-terminated.\n");
    for (char* p (result), *q (p); p != r; p = ++q) {
      while (q != r && *q != '\n') ++q;
  ...
  const char* parser::parse_from_postagged_tostr (char* result, size_t len)
  { return parse_from_postagged (result, len)->print_tostr (_opt.input, _opt.verbose < 0); }
  const sentence_t* parser::read_result (char* result, size_t len) {
    _s->clear (true);
    if (! len) len = std::strlen (result);
    //
    char *r = result + len - 4;
  ...
  void parser::_batch () {
    ...
    if (_opt.input == RAW) {
      if (! _tagger)
        err (1, HERE "fail to invoke MeCab; you may want to set [-d].\n");
      char header[1024];
      char*  line = 0;
      size_t read = 0;
      ...
      close (fd);
      if (avail == 0 && q == q_end)
        err (1, HERE "set a larger value to IOBUF_SIZE.\n");
      if (MODE != PARSE) {
        std::fprintf (stderr, "\r%s: %ld sent. processed.", mode[MODE], n);
        std::fclose (_writer);
      }
    ...
    switch (_opt.learner) { // they should be newed earlier
      ...
      case MAXENT:
        ...
        switch (_maxent_opt.algo) {
          ...
          default: err (1, HERE "MAXENT optimizer disabled.\n");
        }
        _libme->train ();
...
#ifdef USE_JUMAN_POS
    if (i < NUM_FIELD)
      err (1, HERE "# fields is less than %d.\n", NUM_FIELD);
#endif
  }
  ...
  void parser::_set_token_dict () {
    ...
    if (stat (dict.c_str (), &st) != 0) {
      ...
      if (! reader) err (1, HERE "no such file: %s\n", _opt.train);
      char* line = 0;
      size_t read = 0;
      ...
      if (num_particle_features == 0)
        err (1, HERE "no particles found in %s\n"
    ...
    switch (_opt.learner) { // they should be newed earlier
      ...
      case SVM:
        ...
        if (_tiny_param.kernel_type != TinySVM::POLY || _tiny_param.degree > 4)
          err (1, HERE "only polynomial kernel [-t 1] of [-d] <= 4 is supported in SVM.\n");
#endif
        break;
      case MAXENT:
#ifdef USE_MAXENT
  ...
  void parser::_setup_classifier (const input_t in, int argc, char ** argv) {
    ...
    if (_opt.mode == BOTH) { // induce an appropriate classifier from input model
      ...
      if (! fp || std::feof (fp))
        err (1, HERE "no model found: %s; train a model first [-t 0]\n",
             model.c_str ());
      switch (std::fgetc (fp)) {
        case  0 :
