  ...
  void erase(WordIndex e, WordIndex f)
    {
      CPPair *p=find(e,f);
      if(p)
	*p=CPPair(0,0);//erase makes the probability 0. wow. this is the problem
    };
...
public:
  ...
  void smoothedL0NormalizeTable(const vcbList&engl, const vcbList&french, bool);
...
public:
  ...
  // to norlmalize the table i.e. make sure P(fj/ei) for all j is equal to 1 
	...
	void smoothedL0NormalizeTable(const vcbList&engl, const vcbList&french);
	//smoothed l0 optimization by projected gradient descent
