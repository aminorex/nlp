  ...
  for(int it=1; it <= noIterations; it++){
    ...
    if( OutputInAachenFormat==1 )
      tTable.printCountTable(tfile.c_str(),Elist.getVocabList(),Flist.getVocabList(),1);
		if (M3_L0 ==1 && it >=2 )
			{
				cout<<"smoothed normalization model 3"<<endl;
				tTable.smoothedL0NormalizeTable(Elist, Flist,1);
			}
		else
			{
...
#define TEST_ARGS  *testPerp, *testViterbiPerp, *testHandler, dump_files, test_alignfile.c_str(),false, modelName,final
    ...
    if( dump_files&&OutputInAachenFormat==1 )
      tTable.printCountTable(tfile.c_str(),Elist.getVocabList(),Flist.getVocabList(),1);
		if (M3_L0 ==1 && trainingString[it] == '3')
			{
				cout<<"smoothed normalization model3"<<endl;
				tTable.smoothedL0NormalizeTable(Elist, Flist,1);
			}
		else if (M4_L0 ==1 && trainingString[it] == '4')
			{
                cout<<"smoothed normalization model4"<<endl;
				tTable.smoothedL0NormalizeTable(Elist, Flist,1);
