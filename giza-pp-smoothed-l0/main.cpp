...
GLOBAL_PARAMETER(float, MINCOUNTINCREASE,"minCountIncrease","minimal count increase",PARLEV_OPTHEUR,0.0);
...
GLOBAL_PARAMETER(double, ARMIJO_BETA,"armijo_beta","pgd optimization parameter beta used in armijo line search. Defaut is 0.5",PARLEV_EM,0.5);
GLOBAL_PARAMETER(double, ARMIJO_SIGMA,"armijo_sigma","pgd optimization parameter sigma used in armijo line search. Default is 0.5",PARLEV_EM,0.5);
GLOBAL_PARAMETER(double, ETA,"eta","pgd optimization parameter eta used in armijo line search. Default is 0.5",PARLEV_EM,0.5);
GLOBAL_PARAMETER(double, L0_BETA,"smoothed_l0_beta","optimiztion parameter beta that controls the sharpness of the smoothed l0 prior. Default is 0.5",PARLEV_EM,0.5);
GLOBAL_PARAMETER(double, L0_ALPHA,"smoothed_l0_alpha","optimization parameter alpha that controls the weight of the smoothed l0 prior. Default is 10.0",PARLEV_EM,10.0);
GLOBAL_PARAMETER(int, NUM_PGD_ITERATIONS,"num_pgd_iterations","number of pgd iterations we need to carry out. Default is 50. ",PARLEV_EM,50);
GLOBAL_PARAMETER(bool, M1_L0  ,"m1_l0","to use l0 prior for m1 training or not. 1: use l0 prior. Default is 1",PARLEV_EM,1);
//GLOBAL_PARAMETER(bool, M2_L0  ,"m2_l0","to use l0 normalization for m2 or not",PARLEV_EM,0);
GLOBAL_PARAMETER(bool, HMM_L0  ,"hmm_l0","to use l0 prior for hmm training or not. 1: use l0 prior. Default is 1",PARLEV_EM,1);
//GLOBAL_PARAMETER(bool, M3_L0  ,"m3_l0","to use l0 normalization for m3 or not",PARLEV_EM,0);
//GLOBAL_PARAMETER(bool, M4_L0  ,"m4_l0","to use l0 normalization for m4 or not",PARLEV_EM,0);
...
/*
double ARMIJO_BETA = 0.1;
double ARMIJO_SIGMA = 0.0001 ;
double L0_BETA = 0.5;
double L0_ALPHA = 20 ;
int NUM_PGD_ITERATIONS = 20;
double ETA = 0.1;
*/
...
void printGIZAPars(ostream&out)
{
  ...
  /*
  out << "parameters for smoothed l0 optimization:\n"
         "--------------------------------------\n";
  printPars(out,getGlobalParSet(),PARAM_SMOOTHED_L0);
  out << '\n';
  */
}
...
{ 
  ...
  model1 m1(CorpusFilename.c_str(), eTrainVcbList, fTrainVcbList,tTable,trainPerp, 
   ...
   if(ReadTablePrefix.length() )
     ...
     {	
		cout<<"we are starting the training of model 1"<<endl;
       // initialize model1
	 ...
	 //m1.initialize_table_uniformly(*corpus);	//I'm adding this line. I don't know why it was not initialized uniformly
...
int main(int argc, char* argv[])
{
  ...
  initGlobals() ;
  cout<<"we are about to read the config file "<<endl;
  ...
  if( MAX_SENTENCE_LENGTH > MAX_SENTENCE_LENGTH_ALLOWED )
	...
	cout<<"we are about to start training"<<endl;
